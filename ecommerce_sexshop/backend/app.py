...
def get_db():
    ...
    if db is None:
        ...
        # Corrección: Modificamos la tabla para soportar múltiples imágenes
        cursor.execute("PRAGMA table_info(products)")
        columns = [column[1] for column in cursor.fetchall()]
        if 'image_urls' not in columns:
            cursor.execute("ALTER TABLE products ADD COLUMN image_urls TEXT")
            print("--- Columna 'image_urls' agregada a la tabla 'products'. ---")
...
def get_products():
    ...
    for p in products:
        p_dict = dict(p)
        # Manejar la nueva columna de URLs
...
def update_product(product_id):
    ...
    try:
        ...
        # Manejar imágenes si se envía una lista
...
def create_order():
    ...
    try:
        ...
        db.commit()
        return jsonify({"message": "Order created successfully", "total_price": total_price}), 201
